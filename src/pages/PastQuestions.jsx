...
const PastQuestions = () => {
  ...
  // 🔒 SAFETY: always filter an array
  const filtered = Array.isArray(questions)
    ? questions.filter((q) => {
        const levelMatch = level === "All" || q.level === level;
        const semesterMatch =
          semester === "All" || q.semester === semester;
        const deptMatch =
          department === "All" || q.department === department;
        const yearMatch =
          ...
          filtered.map((q) => (
            ...
            >
              <div className="bg-white rounded-xl p-4 space-y-1">
                <p className="text-sm">
                  <strong>Department:</strong> {q.department}
                </p>
                <p className="text-sm">
                  <strong>Course Title:</strong> {q.title}
                </p>
                <p className="text-sm">
                  <strong>Course Code:</strong> {q.course_code}
                </p>
                <p className="text-sm">
